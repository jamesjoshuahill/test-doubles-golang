...
// Source: github.com/jamesjoshuahill/test-doubles-golang/pegomocks (interfaces: repository)

package pegomocks_test

import (
	pegomocks "github.com/jamesjoshuahill/test-doubles-golang/pegomocks"
...
func (mock *Mockrepository) Query(name string, category string) ([]pegomocks.Record, error) {
	if mock == nil {
		panic("mock must not be nil. Use myMock := NewMockrepository().")
	}
	params := []pegomock.Param{name, category}
	result := pegomock.GetGenericMockFrom(mock).Invoke("Query", params, []reflect.Type{reflect.TypeOf((*[]pegomocks.Record)(nil)).Elem(), reflect.TypeOf((*error)(nil)).Elem()})
	var ret0 []pegomocks.Record
	var ret1 error
	if len(result) != 0 {
		if result[0] != nil {
			ret0 = result[0].([]pegomocks.Record)
		}
		if result[1] != nil {
			ret1 = result[1].(error)
		}
